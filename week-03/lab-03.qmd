...
Most web frameworks are loosely organised around the **Model-View-Controller (MVC)** pattern. MVC divides a program into three layers of responsibility so that changes in one area do not ripple through the others.
...
Flask does not enforce MVC — it imposes no required folder structure and no base classes. But the pattern maps naturally onto every Flask app:

1. A browser sends a request to a URL.
2. Flask's router (the **Controller**) matches the URL and calls the right view function.
3. The view function runs whatever Python logic it needs (the **Model**) — perhaps sorting a list, querying a database, or computing a Fibonacci number.
...
- [Flask Quickstart](https://flask.palletsprojects.com/en/3.1.x/quickstart/) example is a great place to start building flask apps
