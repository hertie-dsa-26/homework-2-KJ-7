...
- [Exercises](#exercises)
- [Organising Flask Applications as Packages](#organising)
- [Adding User Input with Flask-WTF Forms](#forms)
...
1. Explain the advantages of databases over hardcoded data
2. Define SQLAlchemy models that map to database tables
3. Create, read, and query data using Flask-SQLAlchemy
4. Establish relationships between database tables (one-to-many)
5. Translate between SQLAlchemy ORM queries and raw SQL
6. Recognise SQL injection vulnerabilities and how to prevent them
7. **Organise Flask applications** as Python packages with separate modules
8. **Create web forms** using Flask-WTF and handle user input securely

# What are Databases? {#databases}
...
# Example: a blog post without a database {#blog}
...
2. Run the app: `uv run flask --app flaskapp run --debug`

Remember this will run in debug mode only if you have:
...
See Lab 3 for other command line options, such as `flask --app flaskapp run --debug`.

# Why Use a Database in a Web App? {#why}
...
- **Querying Flexibility**: Databases support powerful query languages (e.g., SQL) for retrieving, filtering, and analysing data with complex criteria.
...
Once the database is set up with the above structure, we would normally query it using SQL. For example, to retrieve all blog titles along with their authors and categories, we might use the following SQL query:
...
* `JOIN Authors a ON p.author_id = a.author_id`: an inner join connects the `Posts` table to the `Authors` table using the `author_id` column.
* `JOIN Categories c ON p.category_id = c.category_id`: a second inner join on the `category_id` column.
...
To integrate a database into your Flask app, you do not need to know SQL — there are Python libraries that help you work with databases by using Python classes (rather than having to deal with the database tables directly), i.e. in a 'Pythonic' way.
...
This last point underlines one of the advantages of Flask: it provides support for integrating various types of databases into web applications and does not support any single one of them 'natively'. This means that we can choose a database that fits our application: `SQLAlchemy` allows us to switch between databases without having to change the code in our app or change the way in which we are accessing the database. For example, we might want to start off with an SQLite database (which does not require a server) and later — to deploy an application that requires a server — we could change to MySQL or PostgreSQL.

Let us extend our Flask application to use SQLAlchemy and SQLite for database operations. First, let's install the necessary packages. In your command line, run:

```bash
uv add Flask-SQLAlchemy
```

# Example continued: a blog post (now with database) {#blog_database}
...
**Note:** We use `datetime.now(timezone.utc)` instead of the deprecated `datetime.utcnow()` (deprecated in Python 3.12+).
...
At this point our app is broken! Flask is expecting to be able to query a database instance `db`, but it doesn't yet exist!

To create a new blog post and add it to the database, we first need to add the database according to the model we specified in the code of our main app module (as we did just above).
...
**Note:** We use `datetime.now(timezone.utc)` instead of the deprecated `datetime.utcnow()` (deprecated in Python 3.12+). Make sure to import `timezone` from `datetime`.

This is **not** a db column but a `relationship` to the `BlogPost` model. Under the hood, it performs an SQL query to retrieve all blog posts where the `user_id` matches the user's `id`.
...
db.drop_all()
db.create_all()
user1 = User(name='Just Another User')
...
Now, re-do the result that was printed after that last line, this time by writing the statement directly in SQL. For this, create a new variable called `statement_sql`, to which you assign your SQL query wrapped in the `text` wrapper that you imported from `sqlalchemy`. It should look something like this:
...
**Security Warning:** When using raw SQL queries, be aware of **SQL injection attacks**. Never directly interpolate user input into SQL strings. For example:

```python
# DANGEROUS - Never do this!
query = f"SELECT * FROM blog_post WHERE author = '{user_input}'"

# SAFE - Use parameterized queries
query = text("SELECT * FROM blog_post WHERE author = :author")
result = db.session.execute(query, {"author": user_input})
```

The ORM (e.g., `BlogPost.query.all()`) handles this automatically, which is one reason to prefer it over raw SQL in production code.

::: {.callout-note collapse="true"}
## Solution

Replace `posts=BlogPost.query.all()` with:
...
Also change `post.author.name` in `home.html` to `post.author`.
:::
...
As your Flask app grows with databases, forms, and more routes, it makes sense to restructure from a single module to a Python package. Read this helpful explanation [here](https://explore-flask.readthedocs.io/en/latest/organizing.html) for more info.

## From Module to Package Structure
...
This separation makes code more maintainable and testable. See the [example repository](https://github.com/henrycgbaker/my_flask_app_25) for a complete implementation.

---

# Adding User Input with Flask-WTF Forms {#forms}

To feed your database with user input, use `Flask-WTF`, a wrapper for the `WTForms` library. Install it with:

```bash
uv add Flask-WTF
```

See [this introduction](https://exploreflask.com/en/latest/forms.html) or [part 8](https://www.youtube.com/watch?v=u0oDDZrDz9U&list=PL-osiE80TeTs4UjLw5MM6OjgkjFeUxCYH&index=8) of the Flask YouTube tutorial for more details.
...
**Note:** For production, add validators like `Length(min=1, max=100)`. See [WTForms validators](https://wtforms.readthedocs.io/en/stable/validators/).

## Step 2: Create a Template

In `templates/create_post.html`:
...
{% block content %}
    ...
    <form method="post" action="">
        ...
        <div>{{ form.content.label() }}<br>{{ form.content() }}</div>
        <div>{{ form.submit() }}</div>
    </form>
{% endblock %}
```
...
**Note:** `user_id=1` is hardcoded since we haven't implemented authentication. In production, use `current_user.id` with Flask-Login.

## Step 4: Update Navigation

Add a link to the new post page in `layout.html`:

```html
<li><a href="{{ url_for('new_post') }}">New post</a></li>
```

This completes the foundation for user-driven database interactions!
